...
| [CLI Proxy API Management Center](https://github.com/router-for-me/Cli-Proxy-API-Management-Center) | 可视化 API 管理前端 | 5173 |
...
| 脚本 | 部署内容 | 适用场景 |
|------|---------|---------|
| `deploy.sh` | New API + CPA 管理端（Docker 版） | 全新服务器，需要完整部署两个项目 |
| `deploy-no-docker.sh` | New API + CPA 管理端（源码编译） | 无法使用 Docker 或需要定制修改 |
| `deploy-newapi.sh` | 仅 New API（Docker 版） | 只需要 API 网关，或管理端已在别处部署 |
...
.
├── README.md               # 项目说明文档
├── deploy.sh               # Docker 版一键部署脚本（完整部署，推荐）
├── deploy-no-docker.sh     # 无 Docker 版一键部署脚本（源码编译）
├── deploy-newapi.sh        # 单独部署 New API（Docker 版）
...
- [CLI Proxy API Management Center](https://github.com/router-for-me/Cli-Proxy-API-Management-Center) - API 管理前端
